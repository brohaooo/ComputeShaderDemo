...
class SHADERMODULE_API FSimpleComputeShaderParallel : public FGlobalShader
{
public:
	
	DECLARE_GLOBAL_SHADER(FSimpleComputeShaderParallel);
	SHADER_USE_PARAMETER_STRUCT(FSimpleComputeShaderParallel, FGlobalShader);
...
public:
	...
	static void ModifyCompilationEnvironment(const FGlobalShaderPermutationParameters& Parameters, FShaderCompilerEnvironment& OutEnvironment)
	{
		...
		*/
		OutEnvironment.SetDefine(TEXT("THREADS_X"), NUM_THREADS_SimpleComputeShaderParallel_X);
		OutEnvironment.SetDefine(TEXT("THREADS_Y"), NUM_THREADS_SimpleComputeShaderParallel_Y);
		OutEnvironment.SetDefine(TEXT("THREADS_Z"), NUM_THREADS_SimpleComputeShaderParallel_Z);
...
IMPLEMENT_GLOBAL_SHADER(FSimpleComputeShaderParallel, "/CS_Shaders/CS_Parallel.usf", "MySimpleComputeShader", SF_Compute);

void FSimpleComputeShaderParallelInterface::DispatchRenderThread(FRHICommandListImmediate& RHICmdList, FSimpleComputeShaderParallelDispatchParams Params, TFunction<void(TArray<int32> OutputVals)> AsyncCallback) {
...

	{
		...
		TShaderMapRef<FSimpleComputeShaderParallel> ComputeShader(GetGlobalShaderMap(GMaxRHIFeatureLevel));
		...
		if (bIsShaderValid) {
			...
			FSimpleComputeShaderParallel::FParameters* PassParameters = GraphBuilder.AllocParameters<FSimpleComputeShaderParallel::FParameters>();
			...
			GraphBuilder.AddPass(
				RDG_EVENT_NAME("ExecuteSimpleComputeShaderParallel"),
				PassParameters,
			...
			FRHIGPUBufferReadback* GPUBufferReadback = new FRHIGPUBufferReadback(TEXT("ExecuteSimpleComputeShaderParallelOutput"));
			...
			auto RunnerFunc = [GPUBufferReadback, AsyncCallback](auto&& RunnerFunc) -> void {
				if (GPUBufferReadback->IsReady()) {
					...
					OutArray.Add(Buffer[0]);
					OutArray.Add(Buffer[1]);
					
					GPUBufferReadback->Unlock();

					AsyncTask(ENamedThreads::GameThread, [AsyncCallback, OutArray]() {
						AsyncCallback(OutArray);
					});
